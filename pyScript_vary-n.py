"""
	...
	pyScript_vary-n.py: 
		...
		This script calls successive instances of scriptSingleAgent varying the number of games and generates plots showing
		how this trends. 
...
nMax = 200; 		# Maximum number of games to explore. 
...
# 		case, n). Therefore we need to load the data from the respective files. 
...
plt.figure(); 
plt.fill_between(dataNAgents[:,1], np.add(dataNAgents[:,2], dataNAgents[:,3]), np.subtract(dataNAgents[:,2], dataNAgents[:,3])); 
plt.plot(dataNAgents[:,1], dataNAgents[:,2], 'k'); 
plt.xlabel("Number of games"); 
plt.ylabel("Number of agents"); 

plt.figure(); 
plt.plot(dataNAgents[:,1], np.divide(dataNAgents[:,3], dataNAgents[:,2]), 'k'); 
plt.xlabel("Number of games"); 
plt.ylabel("Number of agents"); 
...
plt.figure(); 
plt.fill_between(dataWealth[:,1], np.add(dataWealth[:,2], dataWealth[:,3]), np.subtract(dataWealth[:,2], dataWealth[:,3])); 
plt.plot(dataWealth[:,1], dataWealth[:,2], 'k'); 
plt.xlabel("Number of games"); 
...
plt.figure(); 
plt.fill_between(dataMA[:,1], np.add(dataMA[:,2], dataMA[:,3]), np.subtract(dataMA[:,2], dataMA[:,3])); 
plt.plot(dataMA[:,1], dataMA[:,2], 'k'); 
plt.xlabel("Number of games"); 
...
plt.figure(); 
plt.fill_between(dataRP[:,1], np.add(dataRP[:,2], dataRP[:,3]), np.subtract(dataRP[:,2], dataRP[:,3])); 
plt.plot(dataRP[:,1], dataRP[:,2], 'k'); 
plt.xlabel("Number of games"); 
